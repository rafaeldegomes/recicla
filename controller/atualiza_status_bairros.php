...
$selectSql = "SELECT status FROM bairros WHERE id_bairro = '$id'";
...
if ($result->num_rows > 0) {
    ...
    // Atualize o status na tabela "bairros"
    $updateSql = "UPDATE bairros SET status = '$newStatus' WHERE id_bairro = '$id'";
    if ($conn->query($updateSql) === TRUE) {
        header('Location: ../bairros.php?status=1');
